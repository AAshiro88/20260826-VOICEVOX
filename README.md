...
- **Dolphin 分流（手動兩階段）**：勾選「用 Dolphin」後，下則回覆由**目前選定的模型**先輸出「回覆指示」（要點、方向、**限制內容**；僅在 3D 演出開啟時才要求附 3D 指令），再由**本機 Ollama 的 Dolphin 系列**模型只在限制範圍內照指示輸出正文（如角色扮演、開放式對話）。指示不存入逐字稿與記憶；渲染一律在本機 Ollama 進行，不上雲端；Dolphin 呼叫失敗時自動回退一般模型
...
| 模型列 | 模型 Combobox（可編輯+即時篩選，右側有文字輸入框可過濾關鍵字）→ 介面語言切換（保留目前 chat 的語言）→ Dolphin 開關＋模型輸入（勾選後下則回覆走「強模型規劃指示 → 本機 Dolphin 受限執行」，Dolphin 不讀原文只看指示）→ 停止朗讀 |
...
使用者在輸入框送出訊息（send_message）
  → 背景執行緒 chat_worker()
      ...
      → （勾選 Dolphin 時）兩階段：強模型輸出「回覆指示」→ 本機 Ollama Dolphin 依指示受限輸出最終回覆
