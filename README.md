...
- **Dolphin 分流（手動兩階段）**：勾選「用 Dolphin」後，下則回覆由**目前選定的模型**先規劃「內容大綱／要點方向」，再交給**本機 Ollama 的 Dolphin 系列**模型渲染成最終文字（如角色扮演、開放式對話）。大綱不存入逐字稿與記憶；渲染一律在本機 Ollama 進行，不上雲端；Dolphin 呼叫失敗時自動回退一般模型
...
| 模型列 | 模型 Combobox（可編輯+即時篩選，右側有文字輸入框可過濾關鍵字）→ 介面語言切換（保留目前 chat 的語言）→ Dolphin 開關＋模型輸入（勾選後下則回覆走「強模型規劃 → 本機 Dolphin 渲染」）→ 停止朗讀 |
...
使用者在輸入框送出訊息（send_message）
  → 背景執行緒 chat_worker()
      ...
      → （勾選 Dolphin 時）兩階段：強模型輸出大綱 → 本機 Ollama Dolphin 渲染成最終回覆
