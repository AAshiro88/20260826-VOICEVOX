...
- **Dolphin 分流（手動兩階段）**：勾選「用 Dolphin」後，下則回覆由**目前選定的模型**先輸出「回覆指示」（要點、方向、**限制內容**與預設 3D 指令），再由**本機 Ollama 的 Dolphin 系列**模型只在限制範圍內照指示輸出正文（如角色扮演、開放式對話）。指示不存入逐字稿與記憶；渲染一律在本機 Ollama 進行，不上雲端；Dolphin 呼叫失敗時自動回退一般模型
