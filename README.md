...
| 模型列 | 模型 Combobox（可編輯+即時篩選，右側有文字輸入框可過濾關鍵字）→ 介面語言切換（保留目前 chat 的語言）→ 翻譯引擎切換（Google／Ollama，選 Ollama 時右側可自選本機翻譯模型，影響非日語回覆的日文朗讀稿）→ Dolphin 開關＋模型輸入（勾選後強模型先生成回覆，再由 Dolphin 依後製提示詞潤飾）→ 停止朗讀 |
