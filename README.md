...
- **Dolphin 後製潤飾**：勾選「用 Dolphin」後，下則回覆由**目前選定的模型**先正常生成（含 3D 指令），再由**本機 Ollama 的 Dolphin 系列**模型依寫死的後製提示詞對原文做詞彙／語氣潤飾（可微調句式、保留原意，禁止增刪 `[3d]` 區塊）。強模型為 `[3d]` 指令唯一來源；後製提示詞為空時跳過 Dolphin。Dolphin 呼叫失敗時自動回退一般模型
...
ollama pull dolphin-llama3   # 使用「用 Dolphin」分流時需要（渲染階段一律走本機 Ollama）
```
...
| 模型列 | 模型 Combobox（可編輯+即時篩選，右側有文字輸入框可過濾關鍵字）→ 介面語言切換（保留目前 chat 的語言） |
| 翻譯列 | 翻譯引擎切換（Google／Ollama，選 Ollama 時可自選本機翻譯模型，影響非日語回覆的日文朗讀稿）→ Dolphin 後製開關＋模型輸入（勾選後強模型先生成回覆，再由 Dolphin 依後製提示詞潤飾）→ 停止朗讀 |
...
使用者在輸入框送出訊息（send_message）
  → 背景執行緒 chat_worker()
      ...
      → （勾選 Dolphin 時）兩階段：強模型正常生成回覆（含 [3d]）→ 本機 Ollama Dolphin 依後製提示詞潤飾原文
      → call_llm() 呼叫 Ollama 或 OpenRouter（未分流時直接呼叫）
